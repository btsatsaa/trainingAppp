  ...
  useEffect(() => {
    const filtered = lessons.filter((lesson) => {
      ...
      const addressMatch =
        ...
        lesson.lesson_address.toLowerCase().includes(searchQuery.toLowerCase());
      const categoryFilterMatch =
        selectedCategory === "" || lesson.category === selectedCategory;
      const districtFilterMatch =
        selectedDistrict === "" || lesson.lesson_address.toLowerCase().includes(selectedDistrict.toLowerCase());
      return (
		 ...
		 Уучлаарай таны хайсан сургалт одоогоор байхгүй байна!
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-6 sm:gap-5">
            {filteredLessons.map((lesson) => (
              <ProjectSingle key={lesson.id} lesson_id={lesson.id} {...lesson} />
            ))}
          </div>
